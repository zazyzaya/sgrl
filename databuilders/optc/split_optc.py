...
'''
Stolen from Euler repo; ACM branch
'''
...
def split_all_flows(nmap_f, fold=TRAIN):
    files = glob.glob(RAW_OPTC+fold+'**/*.gz')
    print(len(files))
    print(files[0])
...
def split_all_sessions(fold=TRAIN):
    files = glob.glob(RAW_OPTC+fold+'**/*.gz')
    print(len(files))
    print(files[0])
    ...
    with Manager() as manager:
        ...
        # Start all jobs
        tasks = [
            p.apply_async(copy_user_sessions, (f, lock, i, len(files)))
            for i,f in enumerate(files)
        ]
        print("Queued %d jobs" % len(tasks))
...
def copy_user_sessions(in_f, lock, i, tot):
    in_f = gzip.open(in_f)
    ...
    while line:
        ...
        if datum['object'] == 'USER_SESSION':
            props = datum['properties']

            # Ignore non-human user actions
            if not ((user := props['user']).startswith('SYSTEMIACOM')):
                line = in_f.readline()
                prog.update()
                continue
            ...
            src = host_to_idx(host)
            dst = user
            snapshot = get_snapshot(ts)

            io_buffer[snapshot] += '%f,%d,%s,0\n' % (ts,src,dst)
            buffer_contents += 1

            # Try to minimize syncrhonization
            if buffer_contents > MAX_BUFFER:
                for f_num,out_str in io_buffer.items():
                    fname = USR_SPLIT+str(int(f_num)) + '.csv'
